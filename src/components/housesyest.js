...
class Houses extends React.Component {
	...
	state = {
		urls: ["Tzff0e1XegM","iMQzGShXdGo","Tzff0e1XegM","iMQzGShXdGo","Tzff0e1XegM","iMQzGShXdGo"],
		array1: ['1','2','3','4','5','6'],
		type:'All Types',
		types:[],
		priceM : 1000,
...
priceMax(event){
	if (event.target.value !=""){
		this.state.priceM = event.target.value
	}
	else if( event.target.value == ''){
		this.state.priceM = 1000
	}
				...
				<div className="grid map">
					...
					<div className="grid four large">
						...
						this.state.houses.map((house,index) => (
							<Thumbnail house_id = {this.state.urls[index]} house_city ={house.city} house_region ={house.region} house_price ={house.price} house_rating ={house.rating} house_image ={house.image} house_bedrooms ={house.bedrooms} house_title ={house.title} house_type_name ={house.type.name} />
						))}
					</div>
					<div className="map">
						<GoogleMap
